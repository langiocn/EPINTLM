...
class EPIModel(nn.Module):
    def __init__(self):
        ...
        self.enhancer_sequential = nn.Sequential(nn.Conv1d(in_channels=1280, out_channels=64, kernel_size=40),
                                                 nn.ReLU(),
                                                 nn.MaxPool1d(kernel_size=20, stride=20),
                                                 nn.BatchNorm1d(64),
                                                 nn.Dropout(p=0.5)
                                                 )
        self.promoter_sequential = nn.Sequential(nn.Conv1d(in_channels=1280, out_channels=64, kernel_size=40),
                                                 nn.ReLU(),
                                                 nn.MaxPool1d(kernel_size=20, stride=20),
                                                 nn.BatchNorm1d(64),
                                                 nn.Dropout(p=0.5)
        ...
        self.gru1 = nn.GRU(input_size=64, hidden_size=32, bidirectional=True, num_layers=2)
        self.gru2 = nn.GRU(input_size=64, hidden_size=32, bidirectional=True, num_layers=2)
    ...
    def forward(self, enhancer_ids, promoter_ids, gene_data):
        ...
        enh_out, _ = self.gru1(enhancers_output.permute(2, 0, 1))
        prom_out, _ = self.gru2(promoters_output.permute(2, 0, 1))
